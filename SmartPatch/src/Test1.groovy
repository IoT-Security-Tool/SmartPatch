...
appdir = new File("../sourceFile/Test")
if (appdir !=null && appdir.exists()&& appdir.isDirectory()){
    ...
    if(files !=null && files.length > 0){
        ...
        for (int i = 0; i < files.size(); i++){
            ...
            appfilepath = "../sourceFile/Test/"+filename+".groovy"
            ...
            childLine=[]
            childEventlist=[]
            appentity.getKey(configlist,slurper,eventflag,beginnum,keyline,notduplicate,installedLinenumber,time_init_num,childEventlist,childLine)
            appentity.solveline(after_appsourcefile,keyline)
            appentity.initSource(headfile,headfile_sun,after_appsourcefile,outfile,eventflag,keyline,notduplicate,installedLinenumber,time_init_num,i+1,childEventlist,childLine)
...
def generate(sourcefile,output){
    ...
    def createregex = '.*createEvent\\s*\\(.*'
    def sendregex = '.*sendEvent\\s*\\(.*.*'
    def defcreate = '.*def createEvent.*'
    def defsend = '.*def sendEvent.*'
    List jsonlist = []
    sourcefile.eachLine { line ->
        linenum++
        if (line.trim().length()>=2 &&line.trim().substring(0, 2) != "//"){
            ...
            if (line.matches(createregex) && !line.matches(defcreate)) {
            ...
            if (line.matches(sendregex) && !line.matches(defsend)) {
                //xx?xx:
                //xx
                def param = extractParam(sourcefile, linenum)
                if(param.length() != 0 && line.toString().charAt(line.size()-1) == ":"){
                    List arglist = []
                    arglist = line.toString().split("\\?")
                    List arg = []
                    //delete :
                    //param = param.substring(0, param.length() - 1)
                    //delete ()
                    param = param.trim().substring(1, param.length() - 1)
                    arg = param.split(",")
                    def str = line.toString().replaceAll(arglist[0],"")
                    //delete ?
                    str = str.substring(1, str.length() - 1)
                    def object = objBeforeSend(str)
                    def json = new JsonBuilder()
                    json {
                        type "sendEvent_v1"
                        linenumber linenum
...
def objBeforeSend(str){
    def obj = ''
    def endFlag = 0
    for (int i = 0; i < str.length()-11; i++) {
        if (str.substring(i, i+9) == "sendEvent" ) {
            endFlag = i
            break
        }
    }
    if(endFlag > 0){
        obj = str.substring(0, endFlag)
    }
    return obj
}
