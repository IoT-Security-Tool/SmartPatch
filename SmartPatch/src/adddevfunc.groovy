...
def addcreatEvent(List l, String str, String beforestr, String afterstr, int num){
    ...
    str = str.trim()
    if(str[0] == "[") str.substring(1,str.length()-1)
...
def addsendEvent(List l, String str, String beforestr, String afterstr, int num){
    ...
    if(beforestr.contains(".")){
        ...
        if(temp.size()==1) {
            l.add("\t\tdef signatureResult"+num+"= null")
            l.add("\t\tsignatureResult"+num+" = "+beforestr+"getSignature("+str+".name,"+str+".value)")
            l.add(str+".put('data',[sign: \"\${signatureResult"+num+"}\"])")
            l.add(beforestr+"sendEvent("+str+")"+afterstr)
        }else{
            def eventmap = dealargs(temp)
            def filedname = eventmap.get("name")
            if(eventmap.get("name") == null)  filedname = eventmap.get("\"name\"")
            ...
            if(eventmap.get("value") == null)  filedvalue = eventmap.get("\"value\"")
            l.add("\t\tdef signatureResult"+num+"= null")
            l.add("\t\tsignatureResult"+num+" = "+beforestr+"getSignature("+filedname+","+filedvalue+")")
            l.add(beforestr+"sendEvent("+str+",data: [sign: \"\${signatureResult"+num+"}\"]"+")"+afterstr)
        }
    }else{
        l.add("def params" + num + " = null")
        l.add("def signatureResult"+ num + " = null")
...
def add(filename,headfile,sourcefile,methodMap,slurper,createEventnumberlist,sendEventnumberlist,createEventlist,sendEventlist,outfile,row){
        ...
        if(createEventnumberlist.indexOf(i+1)!=-1){
            ...
            for (int j = 0; j < createEventlist.size(); j++) {
                ...
                if(tmp.linenumber==i+1){
                    ...
                    //afterobj = []
                    //if(afterTxt.contains(":")){
                    afterTxt = afterTxt.trim()
                    if(afterTxt.length()!=0 && afterTxt[0] == ":"){
                        //afterobj = afterTxt.split(":")
                        index = args.indexOf(":")
    ...
    def addtionallist=[]
    addtionallist.add("def getSignature(name,value){\n" +
            "    def params = null\n" +
            ...
            "}")
    for (int j = 0; j < addtionallist.size(); j++) {
        outlist<<addtionallist[j]
    }
