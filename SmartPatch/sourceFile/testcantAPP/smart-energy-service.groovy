...
def checkAccessToken() {
	...
	if (!atomicState.encoredAccessToken) { 
		...
		def redirectUrl = buildRedirectUrl("requestCode") "requestCode"
...
private refreshAuthToken() {
	...
	if(!atomicState.encoredRefreshToken) {
    	...
    	def authorization = "Basic " + "${appSettings.clientId}:${appSettings.clientSecret}".bytes.encodeBase64()
    	def refreshParam = makePostParams("https:
...
def getHtml() {
    ...
    if (!atomicState.notPaired) {
        ...
        if(EATValidation) {
            ...
            def d = getChildDevice(atomicState.dni)
            def kWhMonth = Math.round(meteringUsage / 10000) / 100 
            def planUsed = 0
            if ( maxLimitUsage > 0 ) {
                planUsed = Math.round((meteringUsage / maxLimitUsage) * 100) 
            } else {
                planUsed = Math.round((meteringUsage/ 1000000) * 100) 
            }
