...
preferences {
	...
	definition(name: "TH1400ZB Sinope Thermostat", namespace: "Sinope Technologies", author: "Sinope Technologies", ocfDeviceType: "oic.d.thermostat") {
		...
		fingerprint manufacturer: "Sinope Technologies", model: "TH1400ZB", deviceJoinName: "Sinope Thermostat", mnmn: "SmartThings", vid: "SmartThings-smartthings-TH1300ZB_Sinope_Thermostat" 
	}
...
def updated() {
  	...
  	if (!state.updatedLastRanAt || now() >= state.updatedLastRanAt + 15000) {
		...
		if(AuxiliaryCycleLengthParam){
			...
			{
				case "15 seconds":
					cmds += zigbee.writeAttribute(0x0201, 0x0404, 0x21, 0x000F)
					break
				case "30 minutes":
					cmds += zigbee.writeAttribute(0x0201, 0x0404, 0x21, 0x0708)
					break
				case "disable":
				default:
					cmds += zigbee.writeAttribute(0x0201, 0x0404, 0x21, 0x0000)
					break
			}
		}
		else{
